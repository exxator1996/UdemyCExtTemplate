...
LaneAssociationType get_lane_change_request(const VehicleType *ego_vehicle, const NeighborVehiclesType *vehicles)
{
    ...
    const float rear_distance_m = rear_vehicle->distance_m;
    ...
    if (rear_distance_m < minimal_distance_m)
    {
        ...
        case LANE_ASSOCIATION_TYPE_LEFT:
        {
            const VehicleType *center_vehicles = get_vehicle_array(LANE_ASSOCIATION_TYPE_CENTER, vehicles);
            const float front_center_distance_m = fabs(center_vehicles[0].distance_m);
            const float rear_center_distance_m = fabs(center_vehicles[1].distance_m);
        ...
        case LANE_ASSOCIATION_TYPE_CENTER:
        {
            ...
            const float front_right_distance_m = fabs(right_vehicles[0].distance_m);
            const float rear_right_distance_m = fabs(right_vehicles[1].distance_m);
            ...
            const float front_left_distance_m = fabs(left_vehicles[0].distance_m);
            const float rear_left_distance_m = fabs(left_vehicles[1].distance_m);
